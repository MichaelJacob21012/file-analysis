...
def counter(word: String, xs: List[String]):  Int = {
 xs.count(_ == word);
}

def occurrences(xs: List[String]): Map[String, Int] = {
  val noDuplicates = xs.toSet
  val pairs = for (word <- noDuplicates) yield (word, counter(word, xs))
  pairs.toMap
}
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
  val words = (lst1 ::: lst2).toSet.toList
  val products = for (word <- words) yield counter(word, lst1) * counter(word, lst2)
  products.sum
}
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
  val num = prod(lst1,lst2)
  val denom = math.max(prod(lst1,lst1),prod(lst2,lst2))
  num/denom.toDouble
}
