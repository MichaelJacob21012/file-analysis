...
def process_ratings(lines: List[String]) : List[(String, String)] = {
  val lists = for(line <- lines) yield line.split(",")
  val pairs = for (list <- lists if (list(2).toInt >= 4)) yield (list(0),list(1))
  pairs.toList
}

def process_movies(lines: List[String]) : List[(String, String)] = {
  val lists = for(line <- lines) yield line.split(",")
  for (list <- lists) yield (list(0),list(1))
}
...
def groupById(ratings: List[(String, String)], 
  ...
  if (ratings.size == 0) m else {
  val userID = ratings.head._1
  val userRatings = for (rating <- ratings if rating._1 == userID) yield rating
  val movies = for (rating <- userRatings) yield rating._2
  val leftRatings = ratings.filterNot(userRatings.toSet)
  val addMap = m + (userID -> movies)
  groupById(leftRatings, addMap)
  }
}
...
def favourites(m: Map[String, List[String]], mov: String) : List[List[String]] = {
  val res = for (item <- m.values if (item.contains(mov))) yield (item diff List(mov))
  res.toList
}
...
def suggestions(recs: Map[String, List[String]], 
  ...
  val countMap = favs.groupBy(identity)
  countMap.toList.sortBy(-_._2.size).map(_._1)
}
...
def recommendations(recs: Map[String, List[String]],
  ...
  if (suggs.size < 2){
    for (sug <- suggs) yield (movs(sug))
  }
  else{
    val suggestTwo = suggs.take(2)
    for (sug <- suggestTwo) yield (movs(sug))
  }
}
